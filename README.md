...
When you run `lnkr init`, paths are automatically saved using `{{remote_root}}` and `{{local_root}}` placeholders (if configured) or `$HOME` for portability.

```toml
# Using placeholders (automatically generated by init)
local = "{{local_root}}/github.com/user/project"
remote = "{{remote_root}}/github.com/user/project"

# Or absolute paths (fallback when no config is set)
local = "$HOME/src/github.com/user/project"
remote = "$HOME/.config/lnkr/github.com/user/project"
...
Note: For `.lnkr.toml` paths, `{{remote_root}}` and `{{local_root}}` placeholders are recommended over `$LNKR_REMOTE_ROOT` environment variables for better portability. The `init` command automatically uses these placeholder formats.
